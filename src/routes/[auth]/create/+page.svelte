<script lang="ts">
  ...
  UPSERT_SSH_CONNECTION,
    ...
    DUPLICATE_SSH_CONNECTION,
    DUPLICATE_DATABASE_CONNECTION,
    DUPLICATE_SFTP_CONNECTION,
    DUPLICATE_BDDTLM_CONNECTION,
    DUPLICATE_BDDHOST_CONNECTION,
    DUPLICATE_BDDX3S_CONNECTION,
    DUPLICATE_WAS_CONNECTION
    } from '$lib/mutations';
    import type { 
      Environment, 
      SSHConnection, 
    ...
    export const newEnvironmentName = writable('');
    export const newHostType = writable('');
    export const errorMessage = writable('');
    export const showInputs = writable(false);
    export const selectedDuplicateEnvId = writable<number | null>(null);
  ...
  const loadConnections = async (env_oid: number) => {
    try {
      ...
      sftpConnections.set(clonedSFTPConnections.map(connection => ({
        ...connection,
        sshConnection: clonedSSHConnections.find(ssh => ssh.con_oid === connection.ssh_con_oid)
      })));
  
      bddtlmConnections.set(clonedBDDTLMConnections.map(connection => ({
        ...connection,
        database: clonedDatabaseConnections.find(db => db.con_oid === connection.bdd_con_oid)
      })));
  
      bddx3sConnections.set(clonedBDDX3SConnections.map(connection => ({
        ...connection,
        database: clonedDatabaseConnections.find(db => db.con_oid === connection.bdd_con_oid)
      })));
  
      bddhostConnections.set(clonedBDDHostConnections.map(connection => ({
        ...connection,
        database: clonedDatabaseConnections.find(db => db.con_oid === connection.bdd_con_oid)
      })));
...
//   const showInputs = writable(false);
  ...
  async function duplicateEnvironment() {
      if ($newEnvironmentName.trim() === '' || $newHostType.trim() === '' || $selectedDuplicateEnvId === null) {
        errorMessage.set('Environment Name and Host Type are required.');
        return;
      }
      try {
        ...
        const newEnvOid = result.data.insert_console_environment_one.env_oid;
        const connectionsResult = await client.query({
          query: GET_CONNECTIONS,
          variables: { env_oid: $selectedDuplicateEnvId }
        });
        const connections = connectionsResult.data.console_environment_by_pk;
        for (const ssh of connections.sshconnections) {
          await client.mutate({
            mutation: DUPLICATE_SSH_CONNECTION,
            variables: {
              newEnvOid,
        ...
        for (const db of connections.databaseconnections) {
          await client.mutate({
            mutation: DUPLICATE_DATABASE_CONNECTION,
            variables: {
              newEnvOid,
              type: db.type,
              free_entry: db.free_entry,
              host: db.host,
              port: db.port,
        ...
        for (const sftp of connections.sftpconnections) {
          await client.mutate({
            mutation: DUPLICATE_SFTP_CONNECTION,
            variables: {
              newEnvOid,
              ssh_con_oid: sftp.ssh_con_oid,
              remote_directory: sftp.remote_directory,
              is_active: sftp.is_active
            }
          });
        }
  
        for (const bddtlm of connections.bddtlmconnections) {
          await client.mutate({
            mutation: DUPLICATE_BDDTLM_CONNECTION,
            variables: {
              newEnvOid,
              bdd_con_oid: bddtlm.bdd_con_oid
            }
          });
        }
  
        for (const bddhost of connections.bddhostconnections) {
          await client.mutate({
            mutation: DUPLICATE_BDDHOST_CONNECTION,
            variables: {
              newEnvOid,
              bdd_con_oid: bddhost.bdd_con_oid
            }
          });
        }
  
        for (const bddx3s of connections.bddx3sconnections) {
          await client.mutate({
            mutation: DUPLICATE_BDDX3S_CONNECTION,
            variables: {
              newEnvOid,
              bdd_con_oid: bddx3s.bdd_con_oid
            }
          });
        }
  
        for (const was of connections.wasconnections) {
          await client.mutate({
            mutation: DUPLICATE_WAS_CONNECTION,
            variables: {
              newEnvOid,
              name: was.name,
              host: was.host,
              port: was.port,
              security_enabled: was.security_enabled,
              username: was.username,
              password: was.password,
              key_store_type: was.key_store_type,
              ssl_config_url: was.ssl_config_url,
              soap_config_url: was.soap_config_url,
              sas_config_url: was.sas_config_url,
              truststore: was.truststore,
              truststore_password: was.truststore_password,
              keystore: was.keystore,
              keystore_password: was.keystore_password
            }
          });
        }
  
        console.log('Duplicated environment and connections:', newEnvOid);
        loadEnvironments();
        newEnvironmentName.set('');
        newHostType.set('');
        ...
        errorMessage.set('Error duplicating environment.');
      }
  };
  ...
  async function upsertConnections() {
    ...
    try {
      ...
      for (const databaseConnection of $databaseConnections) {
       ...
       for (const hostConnection of $bddhostConnections) {
        let baseVariables = {
          env_oid: env_oid
        };
        ...
        for (const tlmConnection of $bddtlmConnections) {
        let baseVariables = {
          env_oid: env_oid
        };
        ...
        for (const x3sConnection of $bddx3sConnections) {
...
<div class="p-4 bg-white dark:bg-gray-800">
    ...
    {#if !$showInputs}
    <form on:submit|preventDefault={($selectedDuplicateEnvId ? duplicateEnvironment : addEnvironment)}>
      <div class="mt-4">
          ...
          <table class="min-w-full text-xs divide-y divide-gray-200 dark:divide-gray-700 border border-gray-300 dark:border-gray-700">
            ...
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              <tr>
                ...
                <td class="px-3 py-2 border-r border-gray-300 dark:border-gray-700"></td>
                <td class="px-3 py-2 border-r border-gray-300 dark:border-gray-700"></td>
                <td class="px-3 py-2 border-r border-gray-300 dark:border-gray-700"></td>
                <td class="px-3 py-2">
                  <Popover.Root>
                      ...
                      <div class="relative p-8 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-300 dark:border-gray-700 max-w-4xl w-full">
                        ...
                        <div class="grid grid-cols-3 gap-4 mt-2 items-center">
                          <div>
                            ...
                            <input type="text" class="bg-gray-100 dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded px-2 py-1 text-gray-700 dark:text-gray-300 w-full" readonly />
                          </div>
                          <div>
      ...
      <div class="bg-white dark:bg-gray-800 p-4 mt-4">
        ...
        <div class="space-y-1 py-0">
          ...
          <div class="flex items-center justify-start">
            ...
            <input type="password" class="border border-gray-300 dark:border-gray-700 rounded py-1 px-2 text-gray-700 dark:text-gray-300 w-2/5 bg-gray-100 dark:bg-gray-900">
          </div>
          <div class="flex items-center justify-start">
